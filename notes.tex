...
\infer[]{%
  \deduce[]{%
    \deduce[]{\textsf{with }\refl^A(n) \Rightarrow M}{\textsf{return } (n_1,\; n_2,\; p.R)}%
  }{\Gamma \vdash \textsf{eqElim } P \textsf{ as } N_1 =_A N_2}%
}{%
  \Gamma \vdash P : N_1 =_A N_2
  \;\; & \;\;
  \Gamma \vdash N_1 : A
  \;\; & \;\;
  \Gamma \vdash N_2 : A
  \;\; & \;\;
  \deduce[]{%
    \Gamma,\; n_1 : A,\; n_2 : A,\; p : n_1 =_A n_2 \vdash R : u
  }{%
    \Gamma,\; A : A \vdash M : [N_1/n_1,\; N_2/n_2,\; P/p]R
  }%
}
\end{gather*}

At first look this is pretty incomprehensible, but let us try to break this down:
\begin{itemize}
  \item $P$ is the proof of equality that we would like to eliminate; specifically it shows that
        $n_1 = n_2$.
  \item The type $R$ is, in some sense, the proof we would like to produce, though
        \emph{before} substituting the actual endpoints.  
        We therefore require that
        \[
           [\,N_1/n_1,\; N_2/n_2,\; P/p\,]R
        \]
        is the type of the goal in the reflexive case.
  \item Since the only way to introduce an equality is with $\refl$, we may pattern-match on $P$
        with the assumption $P \equiv \refl^A(N_1)$.
  \item $M$ is the branch of type $R$ handling that reflexive case.
  \item The remaining premises ensure that each of these pieces is well typed.
\end{itemize}
...
This formulation is equivalent to the general $\mathsf{eqElim}$ rule from
Section~2.3: there we eliminate a proof $P : N_1 =_A N_2$ by pattern matching
on $\mathsf{refl}$ and supplying a branch $M$ of type $R$.  The clause
$J(d; a,a,\mathsf{refl}(a)) \equiv d(a)$ is exactly the reflexive case of
$\mathsf{eqElim}$ specialized to the family $P(x,y,p)$, so both rules express
the same dependent elimination principle for identity types.
...
\end{lemma}
\begin{proof}
By $J$ on $p$ with motive 
$P(x,y,p) := \Id{f(x)}{B}{f(y)}$; 
in the reflexive case we return $\refl(f(x))$.
...
\end{lemma}
\begin{proof}
By $J$ with motive $P'(x,y,p) := (P(x) \to P(y))$; in the reflexive case use the identity function.
\end{proof}
...
\item[Base.] $n \equiv 0$. Then 
$0 + \suc(m) \equiv \suc(m) \equiv \suc(0+m)$ \text{(judgmental equality)}
by the defining equations for $+$, so take $\refl$.
...
\subsection{Introduction, Projections, and Elimination}

An inhabitant of a dependent pair packages a witness and its dependent evidence:
\begin{equation*}
\frac{\ctx \vdash M : A \qquad \ctx \vdash N : B(M)}
     {\ctx \vdash \pair{M}{N} : \Sigma_{x:A} B(x)}.
\end{equation*}

\paragraph{Primitive projections.}
Following the presentation from lecture, the projections
\begin{align*}
\fst &: \Sigma_{x:A} B(x) \to A,\\
\snd &: \prod_{z : \Sigma_{x:A} B(x)} B(\fst(z)),
\end{align*}
are taken as \emph{primitive}.  
They satisfy the computation (\(\beta\)) rules:
\begin{align*}
\fst(\pair{M}{N}) &\equiv M,\\
\snd(\pair{M}{N}) &\equiv N.
\end{align*}

\paragraph{Elimination via projections.}
To use a term $z : \Sigma_{x:A} B(x)$, we eliminate it by reasoning with its 
components $\fst(z)$ and $\snd(z)$.  
Given a family $C(x,y)$ depending on $x{:}A$ and $y{:}B(x)$, producing a term
\[
d(x,y) : C(x,y)
\qquad \text{in context } x{:}A, y{:}B(x),
\]
yields a term
\[
C(\fst(z), \snd(z))
\]
when supplied with $z : \Sigma_{x:A} B(x)$.  
No separate eliminator is needed: elimination proceeds directly by projecting.

\paragraph{$\eta$-law.}
Every dependent pair is determined by its projections:
\begin{equation*}
\pair{\fst(z)}{\snd(z)} \equiv z.
\end{equation*}
This expresses that a value of $\Sigma_{x:A} B(x)$ is judgmentally equal to the pair formed
from its witness and evidence.

\subsection{Aside: a derived let-style eliminator (following NPS~\cite{nps1990})}

Some presentations of dependent pairs, notably Nordström–Petersson–Smith~\cite{nps1990},
treat a let-style eliminator as primitive and define the projections from it.  
In the present notes we follow the lecture and take $\fst$ and $\snd$ as primitive, but a
let-style eliminator can be \emph{derived}.
...
\qquad\text{in context } x{:}A, y{:}B(x),
\]
and $z : \Sigma_{x:A} B(x)$, define the eliminator:
\begin{equation*}
\mathsf{split}(d, z) \;\coloneqq\; d(\fst(z), \snd(z)).
\end{equation*}

It satisfies the expected $\beta$-law:
\begin{equation*}
\mathsf{split}(d, \pair{M}{N}) \equiv d(M,N).
\end{equation*}

This derived operator corresponds to the primitive form used in
Nordström–Petersson–Smith but is not taken as primitive here.
...
\newblock \emph{Programming in Martin-Löf's Type Theory: An Introduction}.
