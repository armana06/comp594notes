...
\begin{gather*}
\infer[]{\ctx \vdash \textsf{fst} \langle M,\; N \rangle = M:A}{\ctx \vdash M:A & \ctx \vdash M:u &  \ctx \vdash N:[M/x]B & \ctx \vdash \Sigma x:A.B} \\
\infer[]{\ctx \vdash \textsf{snd} \langle M,\; N \rangle = N:[M/x]B}{\ctx \vdash M:A &  \ctx \vdash M:u & \ctx \vdash N:[M/x]B & \ctx \vdash \Sigma x:A.B} \\
\end{gather*}
